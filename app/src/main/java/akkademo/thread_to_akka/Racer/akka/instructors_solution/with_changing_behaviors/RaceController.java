...
public class RaceController extends AbstractBehavior<RaceController.Command> {
    ...
    public static class RacerCompletedCommand implements Command {
        private static final long serialVersionUID = 1L;
        private ActorRef<Racer.Command> racer;
        private long racerCompleteTime;
        public RacerCompletedCommand(ActorRef<Racer.Command> racer, long racerCompleteTime) {
        	this.racer = racer;
        	this.racerCompleteTime = racerCompleteTime;
        }
        public ActorRef<Racer.Command> getRacer(){
        	return racer;
        }
    ...
    private Map<ActorRef<Racer.Command> , Long > completedRacers = new HashMap<>();
    private long start;
    ...
    public Receive<Command> createReceive() {
        return newReceiveBuilder()
                ...
                .onMessage(RacerCompletedCommand.class, message -> {
                	completedRacers.put(message.getRacer(), message.racerCompleteTime);
                	getContext().stop(message.getRacer());
                	if(completedRacers.size() == 10) {
