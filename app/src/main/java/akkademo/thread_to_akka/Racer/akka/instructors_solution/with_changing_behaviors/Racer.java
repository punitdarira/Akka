    ...
    public Receive<Command> completed(int raceLength) {
        return newReceiveBuilder()
                .onMessage(PositionCommand.class, message -> {
                    ...
                    message.getController().tell(new RaceController.RacerCompletedCommand(getContext().getSelf(), System.currentTimeMillis()));
                    return Behaviors.same();
                }).onSignal(PostStop.class, signal->{
                	System.out.println("Stop called for " + getContext().getSelf().path());
